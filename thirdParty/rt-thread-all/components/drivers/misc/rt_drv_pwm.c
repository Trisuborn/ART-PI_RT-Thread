...
rt_err_t rt_pwm_enable(struct rt_device_pwm* device, int channel, rt_uint8_t complemantary)
{
    rt_err_t result = RT_EOK;
    struct rt_pwm_configuration configuration = { 0 };
    ...
    configuration.complementary = complemantary ? (RT_TRUE) : (RT_FALSE);
    result = rt_device_control(&device->parent, PWM_CMD_ENABLE, &configuration);

    return result;
}

rt_err_t rt_pwm_disable(struct rt_device_pwm* device, int channel, rt_uint8_t complemantary)
{
    rt_err_t result = RT_EOK;
    struct rt_pwm_configuration configuration = { 0 };
...
static int rt_pwm_get(struct rt_device_pwm* device, int channel, rt_uint32_t* period, rt_uint32_t* pulse)
{
    rt_err_t result = RT_EOK;
    struct rt_pwm_configuration configuration = { 0 };
...
static int pwm_enable(int argc, char** argv)
{
    ...
    if (argc != 4) {
        ...
        rt_kprintf("       pwm_enable <dev> <ch> <complemantary> \n");
        result = -RT_ERROR;
        goto _exit;
    }
...
static int pwm_disable(int argc, char** argv)
{
    ...
    if (argc != 4) {
        rt_kprintf("Usage: pwm_disable pwm1 1 1\n");
        rt_kprintf("       pwm_disable <dev> <ch> <complemantary> \n");
        result = -RT_ERROR;
        goto _exit;
    }
...
static int pwm_get(int argc, char** argv)
{
    ...
    rt_uint32_t period;
    rt_uint32_t pulse;
    result = rt_pwm_get(device, atoi(argv[2]), &period, &pulse);
...
MSH_CMD_EXPORT(pwm_get, pwm_get pwm1 1 100 50);
