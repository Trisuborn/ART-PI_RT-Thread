...
static void lcd_st7789_gpio(void)
{
    ...
    lcd_st7789_mcu_g.Mode = GPIO_MODE_OUTPUT_PP;
    lcd_st7789_mcu_g.Pin = ST7789_CS | ST7789_DC | ST7789_WR | ST7789_RD | LCD_LEDK;
    lcd_st7789_mcu_g.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    HAL_GPIO_Init(ST7789_CTL_GPIO, &lcd_st7789_mcu_g);

    lcd_st7789_mcu_g.Mode = GPIO_MODE_OUTPUT_PP;
    lcd_st7789_mcu_g.Pin = LCD_ST7789_DAT_PIN;
    lcd_st7789_mcu_g.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    HAL_GPIO_Init(ST7789_DAT_GPIO, &lcd_st7789_mcu_g);

    lcd_st7789_mcu_g.Mode = GPIO_MODE_OUTPUT_PP;
    lcd_st7789_mcu_g.Pin = ST7789_RST;
    lcd_st7789_mcu_g.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    HAL_GPIO_Init(GPIOB, &lcd_st7789_mcu_g);
...
static __inline void lcd_st7789_trans_byte(uint8_t byte)
{
#if (USER_USE_SCR == USE_SCR_LCD_ST7789_130)
    ...
    ST7789_DAT_GPIO->ODR = (byte << 8) | (GPIOB->IDR & 0xFF);
    // ST7789_DAT_GPIO->ODR = (byte << 8);
#endif
}
...
{
    uint16_t x, y;
    lcd_st7789_set_region(0, 0, 320 - 1, 240 - 1);
    for (y = 0; y < 240; y++) {
        for (x = 0; x < 320; x++) {
            lcd_st7789_send_pixel_dat(color);
        }
